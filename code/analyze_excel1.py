...
def compute_metrics(recovered, dataset, proc_time=""):
    ...
    for ft in FILE_TYPES_RAW:
        orig = sum(1 for v in dataset.values() if v["type"] == ft)
        valid = sum(1 for f in recovered if f["status"] == "Valid" and f["type"] == ft)
        rate = round((valid / orig * 100) if orig else 0, 2)
        per_type[ft] = {"original": orig, "valid": valid, "rate": rate}

    return {
        "rel": rel, "ret": ret, "rnr": rnr,
        "corrupted": corrupted, "fp": fp,
        "carving_rate": carving_rate, "accuracy": accuracy,
        "per_type": per_type, "proc_time": proc_time
    }
...
def chart_04_per_filetype_s1(ft_data):
    ...
    ph = [ft_data.get(f, {}).get("PhotoRec_S1", 0) * 100 for f in FILE_TYPES_RAW]
    sc = [ft_data.get(f, {}).get("Scalpel_S1", 0) * 100 for f in FILE_TYPES_RAW]
